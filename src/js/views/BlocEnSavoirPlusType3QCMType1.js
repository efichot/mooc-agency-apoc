  ...
  render = () => {
    ...
    const showMoreQuestions =
      showMoreQuestionsConditions
        ...
        .filter(isTrue => isTrue).length > 0 && currentQuestion >= questions.originalQuestions.length;

    const noMoreQuestion = showMoreQuestions
      ...
      <Fade
        ...
        <div className="bloc-en-savoir-plus-type-3-qcm-type-1__answers">
          ...
          {!noMoreQuestion && (
            <PopupBlue
              ...
              {showErrorMessage &&
                error.errorMessage &&
                (error.errorMessage.__html ? (
                  <p className="card-content" dangerouslySetInnerHTML={error.errorMessage} />
                ) : (
                  <p className="card-content">{error.errorMessage}</p>
                ))}
            </PopupBlue>
          )}
        </div>
        ...
        <div
          ...
          <PopupBlue
            ...
            noCross
            styleProps={{
              gridRow: `${grid.rows} / ${grid.rows + 4}`,
              gridColumn: `1 / -1`,
            }}>
            <span className="card-title">{!hideCard && cardToShow.cardTitle}</span>
