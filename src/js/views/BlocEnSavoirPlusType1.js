    ...
    return (
      <Fade
        ...
        <div className="bloc-en-savoir-plus-type-1__cards game" style={{ height }}>
          ...
          <PopupBlue classProps="bloc-en-savoir-plus-type-1__cards--to-show" hidePopup={this.state.hideCard} noCross>
            <span className="card-title">{!this.state.hideCard && cards[this.state.showCard - 1].cardTitle}</span>
            <span className="card-subtitle">{!this.state.hideCard && cards[this.state.showCard - 1].cardSubTitle}</span>
            {!this.state.hideCard &&
              (cards[this.state.showCard - 1].cardContent.__html ? (
                <p className="card-content" dangerouslySetInnerHTML={cards[this.state.showCard - 1].cardContent} />
              ) : (
                <p className="card-content">{cards[this.state.showCard - 1].cardContent}</p>
              ))}
          </PopupBlue>
        </div>
      </Fade>
