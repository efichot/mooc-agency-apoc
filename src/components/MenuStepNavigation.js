      ...
      <div className="menu-step-navigation">
        <div className="menu-step-navigation__frame">
          <div className="menu-step-navigation__title">
            <span>{this.props.showPart1 ? stepNavigation.toggleParts.part2 : stepNavigation.toggleParts.part1}</span>
          </div>
          <UserGameInfos class={`menu-step-navigation`} big={false} />
          {Object.keys(stepNavigation)
            .filter(key => stepNavigation[`${key}`].part === (this.props.showPart1 ? 1 : 2))
            .map((key, i, stepsFiltered) => {
              return (
                <div key={i} className={`menu-step-navigation__step`}>
                  <div className="menu-step-navigation__step--bullet">
                    <FontAwesomeIcon icon={['fas','check-circle']}/>
                  </div>
                  <div className={`menu-step-navigation__step--content ${i === (stepsFiltered.length - 1) ? 'last-step' : ''}`}>
                    <div className="menu-step-navigation__step--name"><span>{stepNavigation[`${key}`].name}</span></div>
                    <div className="menu-step-navigation__step--durations"><span>Détails min: 3 / 7 / 3 / 3</span></div>
                  </div>
                </div>
              )
            })
